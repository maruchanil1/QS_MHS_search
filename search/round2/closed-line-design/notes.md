...
| trigonometric displacements, Solov'ev | Ξ(λ,t) e^{ins} with (λ,t)-degrees (2,3) [n = 1,2,3; both parameter sets] and (3,4) [n = 1,2,3]: mode 1 → only the two mode-1 Killing combinations (translation, rotation about a horizontal axis); modes 2, 3 → nullspace 0 | verified_symbolically | `lindef_trig_solovev.py`, logs `lindef_trig_n*_*.log` |
| 1-jet test at a point | imposing the four conditions on the 1-jet (C, C′) at one label value is NOT decisive: 3 spurious jet directions (C = 0, C′ ≠ 0 or mixed) survive beside the 2 Killing jets; higher jets never close by count | negative methodological result | `lindef_jet.py` |
